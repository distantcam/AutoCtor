...
internal sealed class ACTR008_CodeAnalyzerTests
{
    [Test]
    [CombinedDataSources]
        ...
        CompilationBuilderFactory builderFactory)
    {
        ...
        var compilation = builder.Build(nameof(ACTR008_CodeAnalyzerTests));

        var compilationWithAnalyzers = compilation.WithAnalyzers([new AddAutoConstructAnalyzer()]);
    ...
    {
        foreach (var codeAnalyzerExample in GetExamplesFiles("ACTR008_CodeAnalyzerExamples"))
        {
